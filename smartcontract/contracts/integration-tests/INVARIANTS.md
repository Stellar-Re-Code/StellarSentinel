...
| INV-X4 | Authorized settlement | A passed, unexpired Funding proposal transfers the treasury's bound asset exactly once, atomically, under the policy version that authorized it; the authorization cannot be replayed, retargeted at another treasury, executed by an unregistered governance contract, or reused across a stale policy | `cross_contract_invariants::funding_proposal_execution_transfers_atomically`, `funding_proposal_fails_when_governance_not_authorized`, `funding_proposal_insufficient_treasury_balance_does_not_execute`, `funding_proposal_replay_and_cross_contract_misuse_is_rejected`, `governance::test::test_funding_proposal_stale_policy_after_signer_change_fails_atomically`, `governance::test::test_funding_proposal_execution_deadline_enforced` |
